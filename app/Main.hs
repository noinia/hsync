...
data FileTreeChanged d f dirDelta =
    FileChanged (Delta f)
...
deriving stock instance (Show d, Show f, Show dirDelta, Show (Delta f)
                        ) => Show (FileTreeChanged d f dirDelta)
deriving stock instance (Eq d, Eq f, Eq dirDelta, Eq (Delta f)
                        ) => Eq (FileTreeChanged d f dirDelta)
deriving stock instance (Generic d, Generic f, Generic dirDelta, Generic (Delta f)
                        ) => Generic (FileTreeChanged d f dirDelta)


data DirDelta cache f = OutdatedCache  (Delta cache)
                      | ContentChanged (Delta cache) (Delta (DirectoryContent cache f))

instance (HasDiff f, HasDiff d) => HasDiff (FileTree (Cache d) f) where
  type Delta (FileTree (Cache d) f) = FileTreeChanged (Cache d) f (DirDelta (Cache d) f)
  ...
  diff (Directory cacheL cntsL) (Directory cacheR cntsR) = case diff cacheL cacheR of
    NoDifference          -> NoDifference
    Difference deltaCache -> Difference . DirectoryChanged $ case diff cntsL cntsR of
        NoDifference      -> OutdatedCache deltaCache
        Difference deltaCnt -> ContentChanged deltaCache deltaCnt
